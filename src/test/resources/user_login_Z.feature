Feature: Sign in to mortgage account
  ...
  Scenario 3: User attempts to sign in with missing credentials
    Given the user is on the sign in page
    When the user attempts to sign in without entering an email address and/or password
    Then the sign in button should be disabled
    And the user should see an error message indicating the missing field(s)
